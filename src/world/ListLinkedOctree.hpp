...
protected:
	...
	uint8_t _depth; // use uint8_t because depth will never exceed 256 
...
class  ListLinkedOctree {
	...
	unsigned int** lastValptr; // storing pointer of the pointer witch storing id in octree corresponding to the last value
...
template <typename T> 
ListLinkedOctree<T>::ListLinkedOctree(const uint8_t &depth) : _octree(depth), _depth(depth), lastValptr(nullptr) {
	_data.reserve( size() ); // reserve max size (octree size)
}

template <typename T>
void ListLinkedOctree<T>::swapWithEnd(unsigned int*& id) {
	std::swap(_data[*id], _data[**lastValptr]);// swap value in data vector
	std::swap(*id, **lastValptr); // swap value id in our octree
}
